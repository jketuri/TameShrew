...
HRESULT DSVL_GraphManager::BuildGraphFromXMLHandle(TiXmlHandle xml_h)
{
	...
	if(e_camera)
	{
		...
		{
			swprintf(mf.sourceFilterName, _bstr_t(e_camera->Attribute("device_name")));
		...
		{
			swprintf(mf.sourceFilterName, _bstr_t(e_camera->Attribute("friendly_name")));
	...
	else if(e_avi)
	{
		...
		swprintf(mf.sourceFilterName, _bstr_t(e_avi->Attribute("file_name")));
